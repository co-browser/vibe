...
import type { AgentConfig, StreamResponse, ExtractedPage, AgentStatus } from '@vibe/shared-types';
...
export async function* sendMessage(message: string): AsyncGenerator<StreamResponse> {
...
export async function saveTabMemory(extractedPage: ExtractedPage): Promise<void> {
  if (!agent) throw new Error('Agent not initialized');
