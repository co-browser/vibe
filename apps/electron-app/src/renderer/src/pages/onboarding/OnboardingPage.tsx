...
export function OnboardingPage() {
  const [currentStep, setCurrentStep] = useState<'welcome' | 'password-import'>('welcome');
  ...
  useEffect(() => {
    ...
    const handleComplete = (result: PasswordImportResult) => {
      addConsoleOutput(`✓ Successfully imported ${result.passwordCount} passwords from ${result.browser}`);
      addConsoleOutput(`Import completed successfully!`);
      setIsImporting(false);
    };

    const handleError = (result: PasswordImportResult) => {
      addConsoleOutput(`✗ Error importing from ${result.browser}: ${result.error}`);
      setIsImporting(false);
    };

    // Register IPC listeners using the electronAPI
    const progressUnsubscribe = window.electronAPI?.ipcRenderer?.on('password-import-progress', (event: any, progress: PasswordImportProgress) => {
      handleProgress(progress);
    });

    const completeUnsubscribe = window.electronAPI?.ipcRenderer?.on('password-import-complete', (event: any, result: PasswordImportResult) => {
      handleComplete(result);
    });

    const errorUnsubscribe = window.electronAPI?.ipcRenderer?.on('password-import-error', (event: any, result: PasswordImportResult) => {
      handleError(result);
    });
    ...
    return () => {
      if (progressUnsubscribe) {
        window.electronAPI?.ipcRenderer?.removeListener('password-import-progress', progressUnsubscribe);
      }
      if (completeUnsubscribe) {
        window.electronAPI?.ipcRenderer?.removeListener('password-import-complete', completeUnsubscribe);
      }
      if (errorUnsubscribe) {
        window.electronAPI?.ipcRenderer?.removeListener('password-import-error', errorUnsubscribe);
      }
    };
  }, []);
  ...
  const handlePasswordImport = async (browser: string) => {
    ...
    try {
      // Use actual IPC call
      const result = await window.electronAPI?.ipcRenderer?.invoke('password-import-start', browser);
      
      if (!result?.success) {
        addConsoleOutput(`✗ Failed to start import from ${browser}: ${result?.error || 'Unknown error'}`);
        setIsImporting(false);
      }
          ...
          <div className="text-center">
            ...
            <div className="space-y-6 mb-8">
              <div className="flex items-start space-x-4 text-left">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">Multi-Tab Browsing</h3>
                  <p className="text-gray-600 dark:text-gray-300">
                    Manage multiple tabs with ease and switch between them seamlessly
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start space-x-4 text-left">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">AI-Powered Chat</h3>
                  <p className="text-gray-600 dark:text-gray-300">
              ...
              <div className="flex items-start space-x-4 text-left">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">Modern Interface</h3>
                  <p className="text-gray-600 dark:text-gray-300">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-8">
      <div className="max-w-4xl w-full bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
        ...
        {/* Terminal Console */}
        <div className="bg-black rounded-lg p-4 mb-6" style={{ minHeight: '200px' }}>
          <div className="flex items-center mb-2">
            ...
            <span className="ml-4 text-gray-400 text-sm font-mono">Password Import Console</span>
          </div>
          <div 
            ref={consoleRef}
          ...
          >
            {consoleOutput.length === 0 ? (
              <div className="text-gray-500">Ready to import passwords. Select a browser above to begin...</div>
            ) : (
