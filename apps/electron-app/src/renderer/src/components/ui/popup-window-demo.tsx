    ...
    const checkWindowStates = async () => {
      try {
        const onboardingResult =
          await window.vibe.interface.isPopupWindowOpen("onboarding");
        const settingsResult =
          await window.vibe.interface.isPopupWindowOpen("settings");
        const aboutResult =
          await window.vibe.interface.isPopupWindowOpen("about");
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700">
      ...
      <div className="mb-4 p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
        ...
        <div className="space-y-1 text-sm">
          <div className="flex items-center space-x-2">
            <span
              className={`w-2 h-2 rounded-full ${windowStates.onboarding ? "bg-green-500" : "bg-gray-400"}`}
            ></span>
            <span className="text-gray-600 dark:text-gray-400">
              Onboarding: {windowStates.onboarding ? "Open" : "Closed"}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <span
              className={`w-2 h-2 rounded-full ${windowStates.settings ? "bg-green-500" : "bg-gray-400"}`}
            ></span>
            <span className="text-gray-600 dark:text-gray-400">
              Settings: {windowStates.settings ? "Open" : "Closed"}
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <span
              className={`w-2 h-2 rounded-full ${windowStates.about ? "bg-green-500" : "bg-gray-400"}`}
            ></span>
            <span className="text-gray-600 dark:text-gray-400">
              About: {windowStates.about ? "Open" : "Closed"}
            </span>
          </div>
        </div>
      </div>
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mt-4">
        Buttons are automatically disabled when their respective windows are
        open. Check the browser console for detailed results.
      </p>
    </div>
  );
}
